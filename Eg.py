...
API_KEY = '418a6d6fd5c3ad8803c93a332068fe26'  # Replace with your actual API key
...
def get_weather_data(city):
    ...
    if response.status_code == 200:
        ...
        return lat, lon, temp, weather_description
    else:
        return None, None, None,None
def generate_irregular_polygon(lat, lon, num_points=6, radius=3.5):
    points = []
    
    for i in range(num_points):
        # Randomly vary the angle between points to create irregularity
        angle = 2 * math.pi * i / num_points + random.uniform(-0.3, 0.3)
        
        # Calculate the new latitude and longitude based on the fixed radius and varying angle
        delta_lat = radius * math.cos(angle)
        delta_lon = radius * math.sin(angle)
        
        # Adjust the center point by delta values to get surrounding points
...
# List of multiple cities for each country
country_cities = {
...
# Function to get between 3 to 15 lat/lon pairs for a country
def get_danger_zone_coords(city):
    lat, lon, _, weather_desc = get_weather_data(city)  # Get lat, lon, and weather description
    
    if lat is not None and lon is not None:
        # Check if the weather condition qualifies as dangerous
        if ("thunderstorm" in weather_desc.lower() or
            "heavy rain" in weather_desc.lower() or
            "snow" in weather_desc.lower() or
            "fog" in weather_desc.lower() or
            "overcast clouds" in weather_desc.lower() or
            "hail" in weather_desc.lower()):
            # Generate surrounding points around the center
            danger_zone_coords = generate_irregular_polygon(lat,lon, num_points=random.randint(3, 10))
            
            # Close the polygon by appending the first point at the en
            danger_zone_coords.append(danger_zone_coords[0])
...
for cities in country_cities.values():  # Directly iterate over the cities list
    for city in cities:
        print(f"Processing danger zone for {city}")
        
        # Get the coordinates for danger zones around the current city
        coords = get_danger_zone_coords(city)  # Only pass one city at a time
        if len(coords) >= 3:
            # Prepare a row for the DataFrame
            row = {"Nation": city}  # Store only city name
            
            # Add lat/lon pairs up to 15 points
            for i, (lat, lon) in enumerate(coords[:15], start=1):
                row[f"Lat{i}"] = lat  # Store latitude
                row[f"Lon{i}"] = lon  # Store longitude
            
            data.append(row)  # Add row to data list
print(data)
# Create a DataFrame and save to Excel
df = pd.DataFrame(data)
df.to_excel("weather_data_danger_zones.xlsx", index=False)
print("Excel file 'weather_data_danger_zones.xlsx' has been created with danger zone data.")
...
def show_map_window():
    ...
    write_date()
    danger_zones = load_danger_zones('weather_data_danger_zones.xlsx')

    # Create tkinter windowpip
    root_tk = tk.Toplevel()
    #root_tk=tk.Tk()
